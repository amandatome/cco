...
class Checkout_Page extends Base_View {
	 ...
	 */
	private function distraction_free() {
		$no_distraction = get_theme_mod( 'neve_enable_checkout_distraction_free', false );

		if ( $no_distraction === false ) {
			return;
		}
		...
		add_filter(
			'body_class',
			function ( $classes ) {
				$classes[] = 'nv-checkout-no-distraction';
		...
		add_filter(
			'neve_filter_view_data_navbar',
			function ( $data ) {
				$data['primary_menu'] = '';
				$data['navbar_class'] = 'nav-center';
		...
		add_filter( 'theme_mod_neve_top_bar_enable', '__return_false' );
